...
DEFAULT_QR_URL = "https://fox.schoomy.com/"  # QRコードURL が空欄のときの既定URL
...
def qr_data_uri(url):
    """QRコードURL から PNG QR を生成し data URI を返す。空欄なら既定URLを使う。"""
    url = (url or "").strip() or DEFAULT_QR_URL
    qr = qrcode.QRCode(
...
def host_block_html(value):
    """主催企業の帯HTML。空欄なら帯ごと出さない（空文字）。CONTACT行と同じ体裁。"""
    s = render_text(value)
    if not s:
        return ""
    return ('  <div class="presented-by-line">'
            '<span class="label">主催</span>%s</div>' % s)
...
def main():
    ...
    # QRコード（空欄なら既定URL）と 主催企業の帯（空欄なら非表示）
    html = html.replace("{{qr_image}}", qr_data_uri(data.get("QRコードURL", "")))
    html = html.replace("{{HOST_BLOCK}}", host_block_html(data.get("主催企業", "")))
